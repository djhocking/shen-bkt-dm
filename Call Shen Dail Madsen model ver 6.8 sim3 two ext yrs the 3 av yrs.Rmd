...
setwd('G:/Conte/Broad spatial modelling/VA/analysis/Dail Madsen 6/ver 6.8 sim/sim3/2 ext yrs then 3 av yrs/')
getwd()
library(reshape2)
library(rjags)
library(plyr)
...
load("Shen Dail Madsen input data.RData") 
```
...
# Add simulation years 
nSimYears = 30
nYears.new <- nYears + nSimYears

# Define climate condition
fall.flow.new <- c(fall.flow, rep(c(-1.5,-1.5,0,0,0), nSimYears/yCycle))    # 2 very low fall flow yrs then 3 av
winter.flow.new <- c(winter.flow, rep(c(1.5,1.5,0,0,0), nSimYears/yCycle))  # 2 very high winter flow yrs then 3 av
spring.flow.new <- c(spring.flow, rep(0,nSimYears))
spring.temp.new <- c(spring.temp, rep(0,nSimYears))

# Add NA to fish count
library(abind)
na.sim.y <- array(NA, dim=c(nSites,nSimYears,nAges,nPasses))
y.new <- abind(y, na.sim.y, along=2)

# Add 0 to Jdate
na.sim.Jdate <- array(0, dim=c(nSites,nSimYears))
Jdate.new <- abind(Jdate, na.sim.Jdate)
                      
# Bundle data
Dat <- list(nSites=nSites, nYears=nYears.new, nAges=nAges, 
            y=y.new, elev=elev, Jdate=Jdate.new, #width=width, 
            fall.flow=fall.flow.new, winter.flow=winter.flow.new, spring.flow=spring.flow.new,
            spring.temp=spring.temp.new)

# INITIAL VALUES
## Provide first year abundance for S matrix
SNew <- array(NA, dim=c(nSites,nYears.new-1,nAges))
SNew[,1,] <- 100
...
ggplot(yearTotalAbund, aes(x=year2, y=total.mean, colour=size2)) +
  ...
  labs(title = "Two extreme years followed by 3 average years: \nYearly total abundance (with 95 % CI) over 72 sites in Shenandoah") +
...
## Among-site variation in trend over 45 years
```{r among-site variation in trend}
yearSiteAbundMean <- adply(out2$N, c(1,2,3), mean)
year3 <- rep(rep(1:45, each=nSites), nAges)
age3 <- rep(c("YOY","adult"), each=nSites*nYears.new)
elev3 <- rep(elev.raw, nYears.new*nAges)

yearSiteAbund <- cbind(yearSiteAbundMean, year3, age3, elev3)
names(yearSiteAbund) <- c("site","year","age","abund.mean","year2","age2","elev")
yearSiteAbund$year2 <- as.numeric(as.character(yearSiteAbund$year2))
# elevation class
yearSiteAbund$elev.class <- factor(ifelse(yearSiteAbund$elev < 400, 'low elevation (< 400m)',
                                   ifelse(yearSiteAbund$elev > 600, 'high elevation (> 600m)', 
                                          'mid elevation (400-600m)')))
yearSiteAbund$elev.class <- ordered(yearSiteAbund$elev.class, 
                                    levels=c("high elevation (> 600m)",
                                             "mid elevation (400-600m)",
                                             "low elevation (< 400m)"))

ggplot(yearSiteAbund, aes(x=year2, y=abund.mean, group=site, colour=elev.class)) + 
  geom_line(size=0.5) +
  facet_wrap( ~ age2, ncol=2) +
  xlab("Year (data are years 1-15 and projections are years 16-45)") +
  ylab("Estimated local abundance (per 100 m)") +
  labs(title = "Two extreme years followed by 3 average years: \nYearly local abundance (with 95 % CI) at each site in Shenandoah") +
  theme(axis.title.y = element_text(size = rel(1.5), angle=90),
        axis.title.x = element_text(size = rel(1.5)),
        axis.text = element_text(size = rel(1.2)),
        plot.title = element_text(size = rel(1.5), colour="blue"),
        strip.text.x = element_text(size=rel(1.7), face="bold"),
        legend.background = element_rect(colour = "black"),
        legend.text = element_text(size = rel(1)),
        legend.title = element_blank(),
        legend.position=c(0.38, 0.85))
```
